...
<?php 
	...
	if( $_SERVER[ "REQUEST_METHOD" ] == "POST" ) {
	    ...
	    if( regex_validate( regex_pass_pattern, $temp_password ) ) return header( "location: /register/register.php?error=password" );
	    if( regex_validate( regex_user_pattern, $temp_username ) ) return header( "location: /register/register.php?error=username" );

	    // Length check
	    if( strlen( $temp_password ) < 5 || strlen( $temp_password ) > 20 ) return header( "location: /register/register.php?error=password" );
	    ...
	    if( $count ) {
	    	header( "location: ../register/register.php?error=exists" ); die();
	    } else {
